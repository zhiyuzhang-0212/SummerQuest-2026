...
- 组织内补充文档：[CS336 Assignment 2 Systems：Profiling（性能分析）实验指南](https://acnc6zeentra.feishu.cn/docx/D3omdgl6NocdKNxNvc5cW7KJnHd)

## 环境与可复现性

| 项目 | 实际记录 |
| --- | --- |
| GPU | NVIDIA GeForce RTX 4090（平台报告总显存 49,140 MiB，约 48 GiB；不是题面标准 24 GiB 卡） |
...
| 测量进程 | 每个矩阵配置独立进程；CUDA 单卡可见；输出路径只保留文件名到公开 metadata |

本机的 GPU 型号是 4090，但暴露显存约 48 GiB。以下 A2-P 数字是这台开发机上的实际观测，
不能当作标准 24 GiB 4090 的可复现上限；显存 OOM 记录也反映了这个实际硬件和模型配置。
...
| train_step | 5 | 107.582, 107.688, 108.960, 107.883, 107.897, 107.534, 108.225, 107.579, 107.550, 108.028 | 107.893 | 0.441 | 0.0041 |
...
选择/缓存以及首次分配显著拉高；warm-up `5` 后首个 measurement 已接近稳态，CV 从
`0.7392` 降到 `0.0098`（见 `train_step_warmup_5.json` 行）。
...
`373.947 ms`，其中外层 `forward` GPU range 为 `348.617 ms`，`backward` range
为 `0.001 ms` 的 CPU-side annotation（其真实 CUDA kernels 由 autograd 子事件承载），
optimizer 为 `24.215 ms`。attention 子阶段的 GPU device time 为：
scores `21.691 ms`（24 calls）、softmax `180.451 ms`（24 calls）、value
`10.771 ms`（24 calls）。这些阶段是嵌套/异步标记，不能把它们与外层 forward/backward
简单相加；它们用于定位归因，完整测量边界以 `profile/measure` 为准。
...
从 operator 汇总看，代表性 trace 中主要 GPU 时间来自 `aten::mm`（507 calls，
...
两张图都从 snapshot 事件重建 active allocation 曲线；因为 recording 在 warm-up 后开启，
图的起点包含 warm-up 后仍存活的模型/输入 baseline。context 128 的曲线几乎平稳在
约 `13.1 GiB`，context 2048 在 attention score/softmax 的大临时张量出现时升到约
`28.4 GiB`。对应 snapshot 中最大的单次 allocation 分别约为 `20 MiB`（context 128）
和 `4,096 MiB`（context 2048），调用类别归为 matmul 与 softmax/reduction。
...
attention 和 softmax 中间量；训练时 backward 要读取这些 saved tensors，同时产生
gradient tensors，所以完整 train_step 比 forward-only 更容易越过显存上限。
